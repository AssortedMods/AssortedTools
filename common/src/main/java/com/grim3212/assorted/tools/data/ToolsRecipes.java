...
public class ToolsRecipes extends ConditionalRecipeProvider {
    ...
    public void buildRecipes() {
        ...
        ShapelessRecipeBuilder.shapeless(this.items, RecipeCategory.BREWING, ToolsItems.FROST_POWDER.get(), 2).requires(ToolsTags.Items.RODS_FROST).unlockedBy("has_frost_rod", has(ToolsTags.Items.RODS_FROST)).save(this.output, recipeKey(ToolsItems.FROST_POWDER.getId()));
        ...
        ShapedRecipeBuilder.shaped(this.items, RecipeCategory.TOOLS, ToolsItems.NEPTUNE_STAFF.get()).define('D', LibCommonTags.Items.GEMS_DIAMOND).define('C', ToolsItems.ICE_CHARGE.get()).define('S', ToolsTags.Items.RODS_FROST).pattern("D").pattern("C").pattern("S").unlockedBy("has_frost_rod", has(ToolsTags.Items.RODS_FROST)).save(this.output, recipeKey(ToolsItems.NEPTUNE_STAFF.getId()));
